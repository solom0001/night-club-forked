...
const CarouselTwo = ({ slides, pushStyle }: CarouselProps) => {
  ...
  return (
    <div
      className={`content-padding items-center h-[400px] md:h-[500px] gap-6 lg:h-[650px] w-full relative flex flex-col ${pushStyle}`}
    >
      ...
      <div className="*:border *:p-1 flex flex-1 flex-row w-fit mt-2 gap-4 mx-auto">
        ...
        >
          <MdOutlineArrowLeft
            size={35}
            className="text(--white) transition-colors  group-hover:text-(--red)"
          />
        </div>
        <div
          onClick={next}
